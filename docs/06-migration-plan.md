...
**Phase 5 — Traffic cutover and DO decommissioning.** Update DNS to point to the AWS CloudFront distribution. Keep DocumentDB and MongoDB in sync for one additional week as a safety net, then cut writes to MongoDB. Once the week is clean, decommission DigitalOcean droplets and the legacy MongoDB instance. Success criterion: AWS handles 100% of traffic for 7 days with SLOs met. Rollback: DNS flip back.
